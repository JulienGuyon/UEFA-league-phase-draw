...
elseif SOLVER == "SCIP"
	...
	error("Invalid SOLVER. Please chose between 'Gurobi', 'SCIP'")
end
...
We also wanted to check the proportion of admissible matches that are actually admissible
	without the day constraints.
"""
function solve_problem_without_day_constraints(selected_team::Team, constraints::Dict{String, Constraint}, new_match::NTuple{2, Team})::Bool
	...
	# Match variables
	# match_vars[i, j] is a binary variable that is equal to 1 if team i plays at home against team j.
	...
	# General constraints
	@constraint(model, [i = 1:36], sum(match_vars[i, i]) == 0) # A team cannot play against itself
	...
	@constraint(model, sum(match_vars[selected_team_idx, home_idx]) == 1)
	@constraint(model, sum(match_vars[away_idx, selected_team_idx]) == 1)
	...
	for (club, club_constraints) in constraints
		...
		for home_club in club_constraints.played_home
			...
			@constraint(model, sum(match_vars[club_idx, home_idx]) == 1)
		end
		for away_club in club_constraints.played_ext
			away_idx = get_club_index_from_team_name(away_club)
			@constraint(model, sum(match_vars[away_idx, club_idx]) == 1)
		end
	end
			...
			for (k, pot_k) in enumerate((teams.potA, teams.potB, teams.potC, teams.potD))
				for (l, team_l) in enumerate(pot_k)
					...
					if team_j.nationality == team_l.nationality && team_idx != opponent_idx
						@constraint(model, sum(match_vars[team_idx, opponent_idx]) == 0)
					end
				end
			end
		end
...
"""
function solve_problem(selected_team::Team, constraints::Dict{String, Constraint}, new_match::NTuple{2, Team}, env::Union{typeof(SCIP.Optimizer)})::Bool
...
function prefilter_admissible_matches(selected_team::Team, opponent_group::NTuple{9, Team}, constraints::Dict{String, Constraint})::Vector{Tuple{Team, Team}}
	# This function is a prefilter to reduce the number of admissible matches
	# It returns only the matches that are likely to be admissible
	prefiltered_matches = Vector{Tuple{Team, Team}}()
	...
	if home_team === nothing && away_team === nothing
		for home in opponent_group
			for away in opponent_group
				...
				# Reject the couple if one of the two opponents has the same nationality as the selected team
				if home.club != away.club &&
				   home.nationality != selected_team.nationality &&
				   away.nationality != selected_team.nationality &&
				   # Reject the couple if one of the teams has already played against two teams of same nationality
				   ...
				   # If the home team and away team are from same nationality, exclude if selected team has already played once against this nationality
				   !(home.nationality == away.nationality &&
					 constraints[selected_team.club].nationalities[home.nationality] >= 1) &&
				   # Reject the couple if one of the opponents has already played against the pot of the selected team its home/away match
				   ...
				   get_team_already_played_home(away, selected_team_pot, constraints) === nothing
					...
					push!(prefiltered_matches, match)
				end
			end
		end
	end
...
"""
function true_admissible_matches(selected_team::Team, opponent_group::NTuple{9, Team}, constraints::Dict{String, Constraint}, env::Union{typeof(SCIP.Optimizer)})::Vector{Tuple{Team, Team}}
	...
	# If no admissible matches are found, we return an empty vector
	if isempty(true_matches)
		@warn "No admissible matches found for $(selected_team.club) against $(opponent_group)"
	end
...
"""
function true_admissible_matches_without_day_constraints(
	selected_team::Team,
	...
	# For each prefiltered match, we check if it is admissible using the solver
	for match in prefiltered_matches
		if solve_problem_without_day_constraints(selected_team, constraints, match)
			push!(true_matches, match)
		end
	end
...
function uefa_draw_sequential()
	...
	elseif SOLVER == "SCIP"
		...
		error("Invalid SOLVER. Please chose between 'Gurobi' or 'SCIP'")
	end
...
"""
function uefa_draw_with_rejection(nb_draw::Int = 1)
	elo_opponents = zeros(Float64, 36, nb_draw)
	uefa_opponents = zeros(Float64, 36, nb_draw)
	matches = zeros(Int, 36, 8, nb_draw)
	@threads for s in 1:nb_draw
		...
		for pot_index in 1:4
			...
			# Order of selected teams in one pot is random
			indices = shuffle!(collect(1:9))
			for i in indices
				selected_team = pot[i]
				...
				for idx_opponent_pot in 1:4
					...
					while !problem_solved
						...
						if isempty(li_prefiltered_matches)
							@error "No prefiltered matches found for $(selected_team.club) against pot $idx_opponent_pot"
							return 1
						end
		...
		for pot_index in 1:4
			...
			for i in indices
				...
				for idx_opponent_pot in 1:4
					...
					home, away = true_admissible_matches_without_day_constraints(selected_team, opponent_pot, constraints)[rand(1:end)]
